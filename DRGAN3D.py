...
class discriminator(nn.Module):
	...
	def forward(self, input):
		...
		fGAN = self.convGAN( fx ).squeeze(4).squeeze(3).squeeze(2)
		fid = self.convID( fx )
		fid = fid.squeeze(4).squeeze(3).squeeze(2)
...
class DRGAN3D(object):
	def __init__(self, args):
		...
		for iB, (sample_x_, _, sample_image_) in enumerate(self.data_loader):
