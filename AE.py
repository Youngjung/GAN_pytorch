...
# torch.nn.AvgPool2d(kernel_size, stride=None, padding=0, ceil_mode=False, count_include_pad=True)
		...
		self.simplefc = nn.Linear( 320*6*6, 320 )

		utils.initialize_weights(self)

	def forward(self, input):
		x = self.conv( input )
		#x = self.simpleconv( input )
		#x = self.simplefc( x.view(x.size(0),-1) )
		if self.name == 'D':
...
class Decoder( nn.Module ):
	def __init__(self, Nz, Np, Ni):
		...
		self.simplefc = nn.Sequential(
			nn.Linear( 320+Np+Ni+Nz, 1024 ),
			nn.BatchNorm1d(1024),
			nn.ReLU(),
			nn.Linear( 1024, 24*24*128 )
		)
...
class DiscoganGenerator(nn.Module):
	def __init__(self):
		super(DiscoganGenerator, self).__init__()
