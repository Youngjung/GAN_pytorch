	...
	def __init__(self, args):
		...
		for iB, (sample_x3D_,sample_y_,sample_x2D_) in enumerate(self.data_loader):
			...
			if iB > nSamples // self.batch_size:
				break
		self.sample_x2D_ = torch.cat( sample_x2D_s )[:nSamples,:,:,:]
		...
		nSpS = int(math.ceil( math.sqrt( nSamples ) )) # num samples per side
		...
		# networks init
		self.G_2Dto3D = generator2Dto3D()
		self.D_3D = discriminator3D()
		...
		self.G_3Dto2D = generator3Dto2D()
		self.D_2D = discriminator2D()
...
#		print('-----------------------------------------------')
	...
	def train(self):
		...
		for epoch in range(self.epoch_start, self.epoch):
			...
			for iB, (x3D_, y_, x2D_ ) in enumerate(self.data_loader):
				...
				D_3D_GAN_real = self.D_3D(x3D_)
				D_3D_loss_GANreal = self.BCE_loss(D_3D_GAN_real, self.y_real_)

				x3D_hat = self.G_2Dto3D(x2D_)
				D_3D_GAN_fake = self.D_3D(x3D_hat)
				...
				D_3D_loss = D_3D_loss_GANreal + D_3D_loss_GANfake
				...
				D_2D_GAN_real = self.D_2D(x2D_)
				D_2D_loss_GANreal = self.BCE_loss(D_2D_GAN_real, self.y_real_)

				x2D_hat = self.G_3Dto2D(x3D_)
				D_2D_GAN_fake = self.D_2D(x2D_hat)
				...
				D_2D_loss = D_2D_loss_GANreal + D_2D_loss_GANfake
				...
				for iG in range(4):
					...
					# simple GAN loss
					x3D_hat = self.G_2Dto3D(x2D_)
					D_3D_fake_GAN = self.D_3D(x3D_hat)
					G_3D_loss_GANfake = self.BCE_loss(D_3D_fake_GAN, self.y_real_)
					G_3D_loss = G_3D_loss_GANfake

					x2D_hat = self.G_3Dto2D(x3D_)
					D_2D_fake_GAN = self.D_2D(x2D_hat)
					G_2D_loss_GANfake = self.BCE_loss(D_2D_fake_GAN, self.y_real_)
					G_2D_loss = G_2D_loss_GANfake

					# cycle loss
					x2D_recon = self.G_3Dto2D(x3D_hat)
					loss_recon2D = self.L1_loss(x2D_recon, x2D_)

					x3D_recon = self.G_2Dto3D(x2D_hat)
	...
	def visualize_results(self, epoch, fix=True):
		...
		""" fixed noise """
		samples_3D = self.G_2Dto3D(self.sample_x2D_)
		samples_2D = self.G_3Dto2D(self.sample_x3D_)

		recon_3D = self.G_2Dto3D(samples_2D)
		recon_2D = self.G_3Dto2D(samples_3D)
