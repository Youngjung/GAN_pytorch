...
def KL_loss( mu, sigma, batch_size ):
	return 0.5*torch.sum(mu**2 + sigma**2 - torch.log(sigma**2)-1)/batch_size
...
class Encoder( nn.Module ):
	def __init__( self, dim_fx, num_id, num_c_expr ):
		...
		self.conv = nn.Sequential(
			...
			nn.BatchNorm2d(512),
			nn.ReLU(),
			nn.Conv2d(512, dim_fx*2, 8 , 1, 1, bias=True),
			nn.Sigmoid(),
		)
	...
	def forward(self, input):
		x = self.conv( input )
		x = x.view(-1, self.dim_fx*2)
		mean = x[:,:self.dim_fx]
		stddev = self.softplus(x[:,self.dim_fx:]) + 1e-6
		return mean, stddev
...
class generator2d3d(nn.Module):
	...
	def forward(self, x_, y_pcode_onehot_, reparam_z):
		mu, sigma = self.Genc( x_ )
		fx = mu + sigma * reparam_z
		xhat2d = self.Gdec2d(fx, y_pcode_onehot_)
		xhat3d = self.Gdec3d(fx, y_pcode_onehot_)

		return xhat2d, xhat3d, mu, sigma
		...
		for epoch in range(self.epoch_start, self.epoch):
			...
			for iB, (x3D_, y_, x2D_ ) in enumerate(self.data_loader):
				...
				for iD in range(self.n_critic) :
					...
					reparamZ = torch.normal( torch.zeros(self.batch_size, self.dim_fx),
												torch.ones(self.batch_size,self.dim_fx) )
					if self.gpu_mode:
					    reparamZ = Variable(reparamZ.cuda())
					else:
					    reparamZ = Variable(reparamZ)

					xhat2d, xhat3d,_,_ = self.G(x2D_, y_random_pcode_onehot_, reparamZ)
				...
				for iG in range( self.n_gen ):
					...
					xhat2d, xhat3d, mu, sigma = self.G(x2D_, y_pcode_onehot_, reparamZ)
					...
					loss_KL = KL_loss(mu, sigma, self.batch_size)
...
#				np.expand_dims(samples_numpy[j],0).dump( filename )
	...
	def interpolate_id(self, opts):
		...
		if len(opts.eval_comment)>0:
			save_dir = os.path.join(self.result_dir, self.dataset, self.model_name, 'interp_id_'+opts.eval_comment) 
		else:
